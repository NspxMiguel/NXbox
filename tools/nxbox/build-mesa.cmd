...
set "NXBOX_ROOT=%CD%"
set "LDFLAGS=WindowsApp.lib"
set "PATH=C:\ProgramData\chocolatey\bin;C:\ProgramData\chocolatey\lib\winflexbison3\tools;%PATH%"
cd .cache\mesa
python ..\meson\meson.py setup build-nxbox --backend=vs2022 --uwp --buildtype=release --prefix="%NXBOX_ROOT%\mesa-install" -Dcpp_std=vc++17 -Dcpp_args="['/D_XBOX_UWP']" -Dc_args="['/D_XBOX_UWP']" -Db_pch=false -Dc_winlibs=windowsapp -Dcpp_winlibs=windowsapp -Dgallium-drivers=d3d12 -Dvulkan-drivers=[] -Dllvm=disabled -Ddraw-use-llvm=false -Dgallium-d3d12-video=disabled -Degl=disabled -Dgles1=disabled -Dgles2=disabled -Dbuild-tests=false
if errorlevel 1 exit /b 1
